...
def main(cfg, resume, opts):
    ...
    model_student = cifar10_model_dict[cfg.DISTILLER.STUDENT][0](
    ...
    if os.path.exists(model_t_path):
        ...
        for epoch in range(60):
            ...
            info = f'teacher_train_Epoch:{epoch}/{60}\t run_time:{run_time:.3f}\t t_loss:{loss_record.avg:.3f}\t comp_loss:{loss_comp_record.avg:.3f}\t dis_loss:{loss_dis_record.avg:.3f}\t t_acc:{acc_record.avg:.2f}\t'
            print(info, flush=True)
        save_train_teacher(model_teacher, t_optimizer, model_t_path, 99)

    # validate teacher accuracy
    teacher_acc, _, _ = validate_t(val_loader, model_teacher)
    ...
    easy_flag = torch.zeros(N).bool().cuda()
    mid_flag = torch.zeros(N).bool().cuda()
    hard_flag = torch.zeros(N).bool().cuda()
    ...
    trainer = trainer_dict[cfg.SOLVER.TRAINER](
        experiment_name, model_teacher, model_student, train_loader_weak, val_loader, cfg
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cfg", type=str,
                        default="/root/code/MLKD/configs/cifar10/dkd/our_MLKD_DKD_vgg16_r20.yaml")

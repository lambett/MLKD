...
def get_cifar10_dataloaders(batch_size, val_batch_size, num_workers, eval=False):
    data_folder = get_data_folder()
    train_transform_weak, train_transform_strong = get_cifar10_train_transform()
    test_transform = get_cifar10_test_transform()

    if eval:
        dataset = datasets.CIFAR10(root=data_folder, train=True, download=True, transform=test_transform)
        train_loader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers)
        return train_loader

    train_set_weak = CIFAR10Instance(
        root=data_folder, download=True, train=True, transform=train_transform_weak
    )
    train_set_strong = CIFAR10Instance(
        root=data_folder, download=True, train=True, transform=train_transform_strong
    )

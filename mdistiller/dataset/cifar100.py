...
from .transformAugment import RandAugment_before
...
def get_cifar100_train_transform():
    ...
    return train_transform_weak, train_transform_strong
...
def get_cifar100_dataloaders(batch_size, val_batch_size, num_workers, eval=False):
    data_folder = get_data_folder()
    train_transform_weak, train_transform_strong = get_cifar100_train_transform()
    test_transform = get_cifar100_test_transform()

    few_ratio = 1.00  # 0.25 0.50 0.75
    print("--------------------------------------------------")
    print("!!!!!!!!!!!!!!!!!!!!!!!!   {}   !!!!!!!!!!!!!!!!!!".format(few_ratio))

    if eval:
        dataset = datasets.CIFAR100(root=data_folder, train=True, download=True, transform=test_transform)
        train_loader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers)
        return train_loader

    train_set_weak = CIFAR100Instance(
        root=data_folder, download=True, train=True, transform=train_transform_weak, few_ratio=few_ratio
    )
    train_set_strong = CIFAR100Instance(
        root=data_folder, download=True, train=True, transform=train_transform_strong, few_ratio=few_ratio
    )
    num_data = len(train_set_weak)
    test_set = datasets.CIFAR100(
        root=data_folder, download=True, train=False, transform=test_transform
    )

    train_loader_weak = DataLoader(
        train_set_weak, batch_size=batch_size, shuffle=True, num_workers=num_workers
    )
    train_loader_strong = DataLoader(
        train_set_strong, batch_size=batch_size, shuffle=True, num_workers=num_workers
    )
    ...
    return train_loader_weak, train_loader_strong, test_loader, num_data
